...
async def init_db():
    ...
    worker_clients_ddl = (
        """CREATE TABLE IF NOT EXISTS worker_clients(
                ...
                min_trade_amount DOUBLE PRECISION DEFAULT 100,
        ...
        """CREATE TABLE IF NOT EXISTS worker_clients(
                ...
                min_trade_amount REAL DEFAULT 100,
...
async def on_withdraw_card(callback: CallbackQuery, state: FSMContext):
    ...
    await send_section_message(
        callback.message,
        "withdraw",
        "💳 Укажите номер карты для вывода средств:",
        reply_markup=profile_back_keyboard(lang, ("❌ Отмена", "wd_cancel")),
    )
    await callback.answer()
...
async def on_withdraw_trc20(callback: CallbackQuery, state: FSMContext):
    ...
    await send_section_message(
        callback.message,
        "withdraw",
        "👛 Укажите адрес кошелька TRC20 USDT:",
        reply_markup=profile_back_keyboard(lang, ("❌ Отмена", "wd_cancel")),
    )
    await callback.answer()
...
async def process_withdraw_card(message: Message, state: FSMContext):
    ...
    if amount is None:
        await send_section_message(
            message,
            "withdraw",
            "❗ Сумма вывода не найдена. Пожалуйста, начните оформление заново.",
        )
        await state.clear()
        return
    user_row = await get_user_row(message.from_user)
    ...
    await send_section_message(message, "withdraw", "✅ Заявка на вывод отправлена на обработку.")
    await state.clear()
...
async def on_my_deal_view(callback: CallbackQuery, state: FSMContext):
    ...
    await send_section_message(
        ...
        "my_deals",
        text,
        reply_markup=my_deal_result_keyboard(deal_id, lang),
    )
...
async def worker_guide_cb(callback: CallbackQuery):
    ...
    text = (
        ...
        "├ После старта клиент автоматически закрепляется за вами как лохматый.\n"
        "├ Делитесь ссылкой в личных чатах, каналах и рекламных объявлениях.\n"
        "╰ Если ссылка изменится, берите актуальную версию только из панели воркера.\n\n"
